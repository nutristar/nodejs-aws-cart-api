...
export class CartController {
  ...
  async checkout(@Req() req: AppRequest, @Body() body) {
    ...
    if (!(cart && cart.items.length)) {
      return { statusCode: HttpStatus.BAD_REQUEST, message: 'Cart is empty' };
    }
    const order = this.orderService.create({ ...body, userId, cartId: cart.id, items: cart.items, total: calculateCartTotal(cart) });
    this.cartService.removeByUserId(userId);
    return { statusCode: HttpStatus.OK, message: 'OK', data: { order } };
  }
...
//     console.log("POOOOOOOOOSSSSSSSSSTTTTTTTTTT!!!!!!!!!!!!!!")
